...
class App {
  ...
  constructor() {
    // Get User Position
    this._getPosition(); //=> to load the map when the page loaded
  ...
  _getPosition() {
    if (navigator.geolocation)
      navigator.geolocation.getCurrentPosition(
        ...
        function () {
          alert('couldent get your location');
        }
      );
  }
  ...
  _loadMap(position) {
    ...
    this.#map = L.map('map').setView(coords, 13); // need to have an html element with id = map
  ...
  _newWorkout(e) {
    ...
    const allPositivs = (...inputs) => inputs.every(inp => inp > 0);
    ...
    if (type === 'running') {
      ...
      if (
        ...
        !allPositivs(distance, duration, cadence)
      )
